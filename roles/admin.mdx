...
sidebarTitle: "Admin"
description: "The Admin role has unrestricted access to all Ferromax ERP modules: dashboard KPIs, product management, sales history, stock adjustments, remito approvals, and invoice OCR."
---
...
| `GET` | `/proveedores/**` | Full supplier management (GET list is also visible to EMPLEADO) |

<Info>
  Endpoints shared with EMPLEADO (e.g., `POST /ventas`, `POST /recepcion`, `POST /recepciones-remito`, `GET /recepciones-remito/mis-recepciones`, `GET /recepciones-remito/{id}`) are accessible to ADMIN as well. See the [Empleado](/roles/empleado) page for the full shared-access list.
</Info>

## Frontend Routes (Admin)

The React application (`App.jsx`) guards admin-only pages with `<ProtectedRoute requiereAdmin>`:
